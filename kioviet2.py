...
def fetch_data(page, page_size, data_type, id):
    headers = {
        'Retailer': retailer, 
    ...
    url_map = {
        ...
        'invoice_details': f"https://publicfnb.kiotapi.com/invoices/{id}"
    }
...
def main(page_size):
    ...
    # """"""""""""""""""" FETCH INITIAL DATA """""""""""""""""""
    initial_invoices_data, total_records_invoices = fetch_data(0, page_size, 'invoices')
    total_pages_invoices = (total_records_invoices // page_size) + 1

    initial_customers_data, total_records_customers = fetch_data(0, page_size, 'customers')
    total_pages_customers = (total_records_customers // page_size) + 1
    
    all_invoices = []
    all_customers = []
    
    # Fetch all invoices data
    for page in range(total_pages_invoices):
        records, _ = fetch_data(page, page_size, 'invoices')
        all_invoices.extend(records)
    
    # Fetch all customers data
    for page in range(total_pages_customers):
        records, _ = fetch_data(page, page_size, 'customers')
